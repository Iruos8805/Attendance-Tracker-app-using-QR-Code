...
class StudentAttendancePageState extends State<StudentAttendancePage> {
  ...
  Future<List<StudentAttendance>> fetchAttendanceDetails() async {
    ...
    if (response.statusCode == 200) {
      ...
      List<StudentAttendance> filteredList = uniqueAttendances
          .where((attendance) => attendance.uid == widget.email)
          .toList();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Student Attendance', style:TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
  ...
  Widget buildAttendanceList(List<StudentAttendance> attendanceList) {
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        return Card(
          elevation: 5.0, // Add elevation to the list items
          margin: EdgeInsets.symmetric(vertical: 8.0),
          child: ListTile(
            title: Text('Attendance ${index + 1}', style: TextStyle(fontWeight: FontWeight.bold),),
            subtitle: Text(
              'Email: ${attendanceList[index].uid}\n'
